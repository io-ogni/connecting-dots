...
const services = [
  {
    ...
    highlights: [
      { text: "self discovery", cls: "marker" },
      { text: "creative child within us", cls: "marker" },
    ],
  },
  {
    icon: Users,
    ...
    highlights: [
      { text: "What kind of leader do you want to be?", cls: "marker" },
    ],
  },
  {
    icon: Clock,
    ...
    highlights: [
      { text: "important vs. what is urgent", cls: "marker" },
    ],
  },
];

type Highlight = { text: string; cls: string };

const escapeRegExp = (s: string) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

function renderDescription(desc: string, highlights?: Highlight[]) {
  if (!highlights?.length) return desc;
  const map = new Map(highlights.map((h) => [h.text, h.cls]));
  const re = new RegExp(`(${highlights.map((h) => escapeRegExp(h.text)).join("|")})`, "g");
  return desc.split(re).map((part, i) =>
    map.has(part) ? (
      <span key={i} className={map.get(part)}>
        {part}
      </span>
    ) : (
      part
...
const Services = () => {
  return (
    <section className="relative py-24 md:py-32 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-brand-violet/8 via-secondary to-primary/5" />
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-brand-violet/6 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4" />
          ...
          {services.map((service, i) => (
            ...
            >
              ...
              <div className="relative z-10">
                <div className="w-14 h-14 rounded-2xl bg-brand-violet/10 flex items-center justify-center mb-6 group-hover:bg-brand-violet/20 group-hover:shadow-lg group-hover:shadow-brand-violet/15 transition-all duration-300">
                  <service.icon className="w-7 h-7 text-brand-violet group-hover:scale-110 group-hover:rotate-3 transition-all duration-300" />
                </div>
                <h3 className="font-heading text-2xl text-foreground mb-4">
                  {service.title}
                </h3>
                <p className="text-foreground font-body leading-relaxed">
                  {renderDescription(service.description, service.highlights)}
                </p>
              </div>
            </motion.div>
          ))}
